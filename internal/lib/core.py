...
OUTPUT_FAMILIES = (
    ...
    OutputFamily("enrichment", "ora_full_tables", r"Enrichment_Full_", (r"ORA_.*_Full",), True),
    OutputFamily("enrichment", "gsea_full_tables", r"03_Enrichment/GSEA/Tables/GSEA_Full_Table_", ("03_Enrichment/.*/GSEA_Full",)),
    OutputFamily("enrichment", "lollipop", r"Lollipop", (r"ORA_.*_Directional",), True),
    OutputFamily("enrichment", "sankey_bubble", r"sankey_bubble", ("Sankey", "sankey_bubble")),
    OutputFamily("enrichment", "cnet", r"Cnet", ("Cnet",)),
    OutputFamily("enrichment", "emap", r"Emap", ("Emap",)),
    ...
    OutputFamily("regulation", "progeny", r"PROGENy|pathway_activity|target_genes", ("Activity_Sample_Scores", "Activity_.*_Descriptive_Contrasts"), True),
    OutputFamily("network", "ppi_hub", r"PPI_Hub", ("Network_Nodes_and_Hubs",), True),
    ...
    OutputFamily("custom", "custom_heatmaps", r"06_Custom.*Heatmap_", ("06_Custom/(?:.*/)?Heatmap_",)),
...
def inventory_legacy_outputs(legacy_root: Path, new_root: Path | None, output_dir: Path) -> tuple[Path, Path, dict]:
    ...
    for spec in OUTPUT_FAMILIES:
        ...
        status = ("improved_and_replaced" if spec.replacement else "preserved") if new_hits else "missing"
        rows.append({
            ...
            "reason": ("Mapped to a consolidated or redesigned standard output" if spec.replacement
                       else "Matched by explicit output-family rule") if new_hits else "No mapped v4/new output was found",
        })
    ...
    summary = {
        ...
        "families": len(rows),
        "missing_families": sum(row["status"] == "missing" for row in rows),
...
def import_counts(source: Path, project_dir: Path, destination_name: str | None = None) -> tuple[Path, dict]:
    ...
    cfg = validate_project(project_dir / "project.yml")
    ...
    if provenance["normalization"] != "raw_counts":
        raise ValueError("DESeq2 import requires normalization: raw_counts")
