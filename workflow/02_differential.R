...
run_differential <- function(context) {
  ...
  if (nrow(stats::model.matrix(design, metadata)) < ncol(stats::model.matrix(design, metadata))) {
    stop("Design matrix has more coefficients than approved samples")
  }
  ...
  for (i in seq_len(nrow(context$contrasts))) {
    ...
    for (profile_name in names(profiles)) {
      ...
      if (identical(mode, "lfc_threshold_test") && !identical(profile_name, names(profiles)[[1L]])) {
    ...
    result$significant <- result[[paste0("significant_", names(profiles)[[1L]])]]
    result$direction <- result[[paste0("direction_", names(profiles)[[1L]])]]
    ...
    volcano <- plot_volcano_publication(result, contrast_id, primary$padj, primary$abs_lfc)
    ...
    selected_genes <- unique(c(head(result$gene_id[result$direction == "Up"], 25L),
                               head(result$gene_id[result$direction == "Down"], 25L)))
